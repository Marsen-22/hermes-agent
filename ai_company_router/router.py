...
# Map agent_id (the router's internal id) → hermes profile to dispatch to.
# Default profile map: mac_hermes → coder, windows_hermes → researcher.
# Override with AGENT_PROFILE_MAP env var (JSON), e.g.
#   AGENT_PROFILE_MAP='{"mac_hermes": "scout"}'
_DEFAULT_PROFILE_MAP = {
    "mac_hermes": "coder",
    "windows_hermes": "researcher",
}


def _profile_for(agent_id: str) -> str:
    overrides = json.loads(os.environ.get("AGENT_PROFILE_MAP", "{}"))
    return overrides.get(agent_id) or _DEFAULT_PROFILE_MAP.get(agent_id, "default")
...
def real_remote_execute(agent_id: str, sub_task: Dict[str, Any], timeout: int = 90) -> Dict[str, Any]:
    """Dispatch sub_task to a real Hermes crewmate.

    Spawns `hermes chat --profile <profile> -q <description>` as a subprocess
    on the same host. Captures stdout, returns the last 2 KB as 'output'.
    Errors are surfaced via the 'status' field, not raised.
    """
    profile = _profile_for(agent_id)
    description = sub_task.get("description", "").strip()
    if not description:
        return {
            "agent_id": agent_id,
            "profile": profile,
    ...
    cmd = ["hermes", "chat", "--profile", profile, "-q", description]
    ...
    try:
        ...
        return {
            ...
            "profile": profile,
            "status": "timeout",
            "output": f"(no response within {timeout}s)",
            "duration_s": timeout,
            "timestamp": started.isoformat(),
        }
        ...
        return {
            ...
            "profile": profile,
            "status": "error",
            "output": f"hermes binary not found: {e}",
            "timestamp": started.isoformat(),
        }
...
def mac_hermes(state: RouterState) -> Dict[str, Any]:
    ...
    if task is None or not any(p["agent"] == "mac_hermes" for p in state["plan"]):
        return {}
    result = real_remote_execute("mac_hermes", {"description": task["description"], "payload": task["payload"]})
    return {
        "results": {"mac_hermes": result},
        "messages": [AIMessage(content=f"mac_hermes [{result.get('profile')}]: {result.get('output', '')[:200]}")],
    }
...
def windows_hermes(state: RouterState) -> Dict[str, Any]:
    ...
    if task is None or not any(p["agent"] == "windows_hermes" for p in state["plan"]):
        return {}
    result = real_remote_execute("windows_hermes", {"description": task["description"], "payload": task["payload"]})
    return {
        "results": {"windows_hermes": result},
        "messages": [AIMessage(content=f"windows_hermes [{result.get('profile')}]: {result.get('output', '')[:200]}")],
    }
