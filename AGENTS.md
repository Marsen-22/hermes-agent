...
Skins live in `ui-tui/src/skins/` and are selectable via `display.skin`. Customize in a YAML user skin under `~/.hermes/skins/`. See existing built-in skins for the variable contract.

## Plugins

Extend Hermes via `hermes_cli/plugins.py` and runtime-discovered packages under `plugins/`, `plugins/memory/`, `plugins/model-providers/`, and dashboard/context/image-gen directories. Plugins must not modify core files; widen the generic plugin surface instead.

## Skills

Skills are reusable markdown procedures under `.hermes/skills/`. Follow SKILL.md frontmatter and authoring standards.
...
These are durable background systems. Use the Hermes CLI or tool APIs; follow their existing config schemas and lifecycle conventions. Do not reimplement scheduling or work-queue logic in skills.
...
Assert invariants and behavior, not snapshot values that break on every schema/model update.
